...
## 다음 마일스톤 메모
- 모바일은 현재 반응형 보정 위에 계속 누적하지 않고, 다음 마일스톤에서 별도 앱 셸(`mobile.html`, 모바일 전용 HUD/입력/렌더 프리셋)로 분리할 계획이다.
- 빌드/배포 구조는 유지하되, 정적 호스팅 시 데스크톱 진입점과 모바일 진입점을 함께 제공하는 방향을 전제로 준비한다.
- 현재는 `index.html`이 휴대폰형 환경을 감지하면 `mobile.html`로 자동 분기한다. 데스크톱 강제 진입이 필요하면 `?desktop=1`을 붙여 확인한다.
- 현재 모바일 셸은 데스크톱 중앙 HUD를 DOM에서 제거하고, 공용 로직이 요구하는 점수/게이지/스킬 식별자만 숨김 기술 패널로 유지한다.
- 이제 공용 세션 조립은 `src/app/create_game_session.js`가 담당하므로, 모바일/데스크톱이 같은 오디오/배경/임팩트/게임 초기화 경로를 공유하는지 함께 확인한다.
- HUD 기본 수치와 `STATUS/INCOMING`도 `src/ui/hud_renderer.js` 공용 렌더 경로를 타므로, 셸별 DOM 차이에도 같은 상태가 표시되는지 함께 확인한다.
- 전투 콜아웃과 DEV PANEL도 `src/ui/feedback_view.js` 공용 경로를 타므로, 모바일 셸에서도 같은 경고/상태 메타가 정상 표시되는지 확인한다.
- 브리핑/결과/최근 전투 카드도 `src/ui/overlay_view.js` 공용 경로를 타므로, 모바일 셸에서 오버레이 열기/닫기와 카드 렌더가 같은 규칙으로 동작하는지 확인한다.
- 브리핑/설정/결과 오버레이 상태 전이도 `src/ui/overlay_controller.js` 공용 경로를 타므로, 모바일 셸에서 일시정지/오디오 재개/오버레이 닫힘 타이밍이 데스크톱과 일치하는지 확인한다.
...
42. 가로 모바일에서도 `#stage`가 `mobile` 클래스를 유지하고, 데스크톱 3컬럼 대신 세로 스택 + PiP AI 보드 구조가 유지되는지 확인
43. 휴대폰형 환경에서 `index.html` 진입 시 자동으로 `mobile.html`로 이동하는지 확인
44. `mobile.html`에서 데스크톱용 3컬럼 HUD 대신 모바일 전용 셸 스타일이 적용되는지 확인
