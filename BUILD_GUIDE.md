...
38. 갤럭시 S26급 세로 화면에서 플레이어 보드 하단까지 전부 보이고, 버튼이 이전보다 확실히 크게 눌리는지 확인
39. 가로 모바일에서 보드가 거의 사라지지 않고, 2행 컨트롤이 하단을 과도하게 덮지 않는지 확인
40. 모바일에서 `.lane.player-lane .board-wrap` 크기가 바뀌면 `playerCanvas`가 즉시 다시 맞춰지고, 회전 후에도 보드가 컨테이너 밖으로 넘치지 않는지 확인
41. 모바일에서 주소창 축소/확장이나 멀티윈도우 높이 변화가 생겨도 `ResizeObserver` 경로로 보드가 다시 맞춰지는지 확인
