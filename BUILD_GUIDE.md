...
46. 모바일 전투 시작 직후 `prestart` 흐림 상태가 사라지고, 데스크톱에서는 AI 보드가 이전처럼 정상 비율로 유지되는지 확인
47. 세로 모바일에서 안내 문구가 짧은 토스트로만 나타나고, 하단 버튼이 전장을 과도하게 덮지 않는지 확인
48. 세로 모바일에서 상단 HUD가 더 낮아지고, 플레이어 보드가 더 크게 왼쪽에 배치되며, `HOLD` 없이 `NEXT`가 세로 스택으로 표시되는지 확인
49. 세로 모바일에서 `NEXT`가 플레이어 보드의 왼쪽 컬럼에 고정되고, 보드가 바로 오른쪽에서 더 크게 보이는지 확인
50. 세로 모바일에서 `NEXT` 컬럼 폭이 더 줄어들고, 메인 보드가 그만큼 더 높고 크게 보이는지 확인
51. 세로 모바일에서 `NEXT / BOARD / AI / STATUS`의 좌우 마진과 간격이 한쪽으로 쏠리지 않고 보기 좋게 정렬되는지 확인
52. 세로 모바일에서 전장 묶음이 과도하게 중앙으로 치우치지 않고, 약간 왼쪽 정렬된 균형으로 보이는지 확인
