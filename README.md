...
- 📲 **Battle Size Clamp**: 모바일 전투 시작 직후 플레이어 보드와 AI PiP는 전용 상한 크기로 다시 맞춰져, 오버레이 해제 후에도 거대 캔버스로 튀지 않는다
...
- ↔️ **Next-Board Two-Column Layout**: 모바일 세로 전장에서는 `NEXT`가 왼쪽 세로 컬럼에 고정되고, 플레이어 보드는 그 바로 오른쪽에 더 크게 배치된다
- 📏 **Narrow Next Column**: 모바일 `NEXT` 컬럼은 더 얇게 줄이고, 메인 보드는 그만큼 더 크게 확장해 전장 가독성을 높인다
- 🧩 **Portrait Margin Polish**: 모바일 세로 전장의 `NEXT / BOARD / AI / STATUS` 컬럼 마진과 간격을 보기 좋게 다시 다듬었다
- ↩️ **Portrait Left Bias Tuning**: 모바일 세로 전장 묶음은 너무 가운데로 몰리지 않도록 약간 왼쪽으로 재정렬됐다
...
4. 데스크톱 강제 확인이 필요하면 `http://localhost:8080/?desktop=1`
...
4. Force desktop page if needed: `http://localhost:8080/?desktop=1`
...
4. デスクトップ版を強制表示したい場合は `http://localhost:8080/?desktop=1`
...
3. 快取問題時強制刷新: `Ctrl+F5`
4. 若要強制查看桌面版可使用 `http://localhost:8080/?desktop=1`
...
3. 缓存问题时强制刷新: `Ctrl+F5`
4. 如需强制查看桌面版可使用 `http://localhost:8080/?desktop=1`
